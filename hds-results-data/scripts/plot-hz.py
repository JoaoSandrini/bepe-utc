...
def plot_hz_bar_by_load(means_by_group: Dict[str, Dict[str, float]], output_dir: Path) -> None:
	...
	ax.set_title("Mean Output Topic Message Frequency - Bar Plot by Load State")
	...
	ax.legend(legend_handles, ["Default Runtime", "Kata Runtime"], loc="upper right")
	fig.tight_layout()
...
def plot_hz_bar_by_runtime(means_by_group: Dict[str, Dict[str, float]], output_dir: Path) -> None:
	...
	ax.set_ylabel("Mean Frequency (Hz)")
	ax.set_title("Mean Output Topic Message Frequency - Bar Plot by Runtime")
	ax.grid(True, axis="y", alpha=0.3)
	for bar, value in zip(bars, values, strict=False):
		if value is None:
			continue
	...
	ax.legend(legend_handles, ["Load Off", "Load On"], loc="upper right")
	fig.tight_layout()
...
def main() -> None:
	...
	bar_dir = args.output_dir
	plot_hz_bar_by_load(means_by_group, bar_dir)
	plot_hz_bar_by_runtime(means_by_group, bar_dir)
